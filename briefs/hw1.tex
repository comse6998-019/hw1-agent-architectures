...
\midrule
1 & \texttt{\#\# Team} & Each member's name and UNI. The full 40-character SHA of the commit
  that contains the submitted code (the README may be committed after it). \\
...
10 & \texttt{\#\# Rubric map} & A table with columns \code{Item}, \code{Where}.
  ...
  \code{Where} is a \code{path:line} or \code{path:start-end} in the code for
  B items, \code{README.md} for C1, and a heading of the report for A items, C2
  and C3. \\
...
\item The four run directories named in the README (\code{runs/radicale},
  ...
  \code{runs/radicale-cutoff}) and their configs. The run directories under
  \code{runs/} are committed; do not git-ignore them.
