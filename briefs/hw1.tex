...
Staff ship these files in this repository: \code{report.schema.json},
\code{config.example.toml} and \code{README.template.md} at the root; the three
...
{\lstset{title={briefs/report.example.json (truncated illustration: 3 of the 18 OWASP rows)}}\lstinputlisting{report.example.json}}
...
\item Each counter in \code{usage_total} equals the sum of that counter over all
  \code{model_call} events, and the sum of \code{input + output} over them
  equals \code{budget.used_tokens} in \code{report.json}.
...
\item The level-2 headings below appear \textbf{in this order and with exactly
  this text}. You may add other headings only as level-3 (\texttt{\#\#\#})
  headings, inside any required section except \texttt{\#\# Run}, whose four
  \texttt{\#\#\#} subsections are fixed.
